pipeline {
    ...
    parameters {  // Defines Parameters as Code
        ...
        string(name: "git_branch", defaultValue: "test", trim: true, description: "Git branch to build Dockerfile from")
        ...
        string(name: "registry_repo", defaultValue: "devinlimo", trim: true, description: "Container Registry URL")
        ...
        password(name: 'secure_api_token', defaultValue: '', description: 'Enter Secure API Token')
    }
    stages {
            ...
            parallel{
                     ...
                     steps{
                         script {
                             if(!env.sysdig_plugin){    
                                 withCredentials([usernamePassword(credentialsId: 'secure_api_token', passwordVariable: 'secure_api_token')]) {
                ...
                stage('Plugin Scan'){  // Scans the built image using the Sysdig Jenkins Plugin
                    steps{
                        script{
                            if(env.sysdig_plugin){
                                sysdigImageScan engineCredentialsId: "${secure_api_token}", imageName: "${registry_url}/${registry_repo}/${docker_tag}", engineURL: "${params.sysdig_url}", policiesToApply: "${params.plugin_policies_to_apply}", bailOnFail: "${params.bail_on_fail}", bailOnPluginFail: "${params.bail_on_plugin_fail}"
                            }
                            else{
                                echo 'Using CLI Scan'
                            }
